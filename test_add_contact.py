# -*- coding: utf-8 -*-
import pytest
from contact import Contact
from contact_application import ContactApplication

@pytest.fixture()
def contact_app(request):
    fixture = ContactApplication()
    request.addfinalizer(fixture.destroy)
    return fixture

def test_add_contact(contact_app):
    contact_app.login(username="admin", password="secret")
    contact_app.create_contact(Contact(firstName="Anna", lastName="Petrova", nickname="annapetr", company="First Company Anna", address="Moscow, Nikitskaya 22", phone_home="111",
                        phone_mobile="1111", phone_work="111111", fax="11231", email="anna@gmail.com", email2="anna2@gmail.com", email3="anna3@gmail.com",
                        webpage="myhome.com", title="title", bday="9", bmonth="April", byear="2001", an_day="16", an_month="January", an_year="2009"))
    contact_app.logout()
